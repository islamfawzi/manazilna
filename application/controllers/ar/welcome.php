...
class Welcome extends Super {
    ...
    function news($news_id = 0){
      ...
      $GLOBALS['title'] = $GLOBALS['news']['title'];
      $GLOBALS['meta_keys'] = $GLOBALS['news']['title'];
      $GLOBALS['meta_desc'] = $GLOBALS['news']['content'];
      _view_ar("pages");
    }
        ...
        if ($id != NULL) {
            ...
            if(is_array($amenities)){
              foreach ($amenities as $id) {
                  $GLOBALS['product']['amenities'][] = $this->db->select('title_ar')->get_where('amenities', ['id' => $id])->first_row()->title_ar;
              }
            }else{
                $GLOBALS['product']['amenities'][] = $this->db->select('title_ar')->get_where('amenities', ['id' => $amenities])->first_row()->title_ar;
            }
            ...
            $GLOBALS['data'] = $this->projects_mod->getProductPagination($start, 'ar');
            $GLOBALS['title'] = "عقاراتنا";
            $GLOBALS['pageIndex'] = 2;
        }
    ...
    function getProjectsAjax($start = 0) {
        ...
        return $view = $this->load->view('site_ar/projects_small', $data);
    }
    /* End products * */
    ...
    function saved_search(){
      ...
      if(!empty($user)){
        ...
        $GLOBALS['index'] = "Saved Search";
      }
      _view_ar('saved_search');

    }

    function mapSearch(){
      $GLOBALS['index'] = "mapSearch";
      $GLOBALS['title'] = "البحث بالخريطية";
      $properties = $this->db->select('id, title_ar, longitude, latitude')->where("active", 1)->get("projects")->result();
    ...
    function addToCompare($id){
      ...
      foreach ($GLOBALS['products'] as &$product) {
          ...
          if(is_array($amenities)){
            foreach ($amenities as $id) {
                $product['amenities'][] = $this->db->where('id',$id)->get('amenities')->first_row()->title_ar;
            }
          }else{
            $product['amenities'][] = $this->db->where('id',$amenities)->get('amenities')->first_row()->title_ar;
          }
      }

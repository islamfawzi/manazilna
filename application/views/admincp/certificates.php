...
<div class="col-sm-10 col-xs-12">
    <div class="forms">
        <h5 class="form_title text-center"><i class="fa fa-edit"></i><?if($add == 1):?>اضافه شهاده <?else:?>&nbsp;تعديل عنصر <?endif;?></h5>
        <?php if($added == 1): ?>
        ...
        <meta http-equiv="Refresh" content="2; URL='<?= base_url() ?>admincp/edit_certificates/back'" />  
        <?elseif ($updated == 2):?>
        <div class="alert alert-danger">
            <i class="fa fa-remove fa-5 font_big"></i>
        ...
        <div class="tab-content">
            <div class="tab-pane fade in active" id="arabic">
                <div class="table-responsive">
                    <form action="<?= base_url() ?>admincp/add_certificates" method="post" enctype="multipart/form-data" accept-charset="UTF-8" id="main_menu">
                        <?if($add != 1):?>
                        <input type="hidden" name="port_id" value="<?= $id ?>"/>
                        <?endif;?>
                        <table class="table">
                            <tr>
                                <td class="label_width input_label"><i class="fa fa-pencil"></i> اسم المشروع :</td>
...
<div class="col-sm-10 col-xs-12">
    <div class="forms">
        <div class="top_select text-center">
            <h5 class="form_title text-center"><i class="fa fa-edit"></i> الشهادات </h5>
            <p class="pull-right">
            <form action="<?= base_url() ?>admincp/edit_certificates" method="post" class="form-inline pull-right" id="ports_form">
            ...
            <p class="text-left">
                <a href="<?= base_url() ?>admincp/add_certificates/<?= $search_id ?>/<?= $search_sub ?>" class="btn btn-success"><i class="fa fa-plus"></i> إضافة شهادة  </a>
            </p>
        </div>
        <?php if ($deleted == 1): ?> 
        <div class="alert alert-success">
        ...
        <?endif;?>
        <div class="table-responsive">
            <form action="<?= base_url() ?>admincp/edit_certificates" method="post">
                <table class="table table-bordered table-hover">
                    ...
                    <tr>
                        ...
                        <td class="text-center"><a href="<?= base_url() ?>admincp/edit_certificates/<?= $port->id ?>/<?= $port->title ?>"><?= $port->title ?></a><br />
                            <a href="<?= base_url() ?>admincp/edit_certificates/<?= $port->id ?>/<?= $port->title_en ?>"><?= $port->title_en ?></a>
                        </td>
                        ...
                        <td class="text-center"><a href="<?= base_url() ?>admincp/edit_certificates/<?= $port->id ?>/<?= $port->title ?>">تعديل </a></td>
                        <?php if(in_array(999, $privacy)): ?>
...
<script type="text/javascript">
    ...
    $('#porto_cat').change(function () {
        ...
        var x = $(this).val();
        xmlhttp.open("POST", "<?= base_url() ?>admincp/get_project_cat/" + x, true);
        xmlhttp.send();
